...
public class TotalSearchController {
	...
	public String totalSearch (ProductVO vo, String pnum, Model m) {
		//한줄평 검색
		/*
		 * List<HashMap> reviews = main.getReviews(vo, pnum); // 리뷰 for(int idx =
		 * reviews.size()-1; idx >= 0; idx--) { String content = (String)
		 * reviews.get(idx).get("BOOKREPORT_CONTENT"); System.out.println(content); int
		 * start = content.indexOf("<blockquote>"); if(start < 0) { continue; } int end
		 * = content.indexOf("</blockquote>");
		 * reviews.get(idx).put("BOOKREPORT_CONTENT",content.substring(start + 12,
		 * end)); System.out.println(content.substring(start + 12, end)); }
		 */
		
		// 도서 검색
		List<ProductVO> result = product.genreKo(vo,pnum);
